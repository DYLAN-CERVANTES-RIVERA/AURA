...
const InsertVistaPersona = async({Id_Persona,Id_Seguimiento,Nombre,Ap_Paterno,Ap_Materno,Genero,Edad,Fecha_Nacimiento,Telefono,Alias,Curp,Remisiones,Capturo,Foto,Img_64},Persona)=>{//FUNCION QUE INSERTA LOS DATOS DE LAS PERSONAS ASOCIADAS AL SEGUIMIENTO
    ...
    let Forencias = await getForenciasOneRegister(Id_Persona);
    ...
    for await(let Forencia of Forencias){
       ...
       await InsertVistaForencia(formDataForencia,contenedorhtml);//Inserta todos los domicilios de las personas del seguimiento
    }
...
const InsertVistaForencia= async({Id_Forencia,Id_Persona,Id_Seguimiento,Descripcion_Forencia,Capturo,Foto_Nombre,Img_64},main)=>{
    let pathImagesPersonas =base_url_js+'public/files/Seguimientos/'+Id_Seguimiento+'/Forencias/';
    let scr='';
    if(Foto_Nombre!='SD'){
    ...
    createElement('div', {id: "ForenciaPersona"+Id_Persona+Id_Forencia, className: 'form-group'}, [],main);
    document.getElementById("ForenciaPersona"+Id_Persona+Id_Forencia).innerHTML=`
    <a class="subtitulo-azul-grueso">Dato.</a>
    <div class="row">
        <div class=" row col-lg-9">
            <div class="col-lg-8">
                <span class="span_rem">Descripcion del dato: </span>
                <span class="span_rem_ans" >${Descripcion_Forencia}</span>
            </div>
        </div>
        <div class="col-lg-3">
            <div class="col-lg-12">
            ...
            <img name="nor" src="${scr}"  width="300"  data-toggle="modal" data-target="#ModalCenterForencia${Id_Persona+Id_Forencia}">
            <div class="modal fade " id="ModalCenterForencia${Id_Persona+Id_Forencia}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                    <img name="nor" src="${scr}"  style="width:1200px; position:relative; top:0px; left:0px; border:opx; overflow:hidden; display:block"data-toggle="modal" data-target="#exampleModalCenter">
                </div>
            </div>  
